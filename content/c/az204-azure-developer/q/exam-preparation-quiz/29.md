...
answers:
    - id: "answer1"
      title: "TaskSuccess"
      correct: true
      explain: "TaskSuccess specifies that output files should only be uploaded when the task completes successfully (exit code 0). Since the requirement is to store successfully converted files in the output container, this condition ensures that only files from successful conversions are uploaded, preventing corrupted or partially processed files from being stored."
    - id: "answer2"
      title: "TaskFailure"
      correct: false
      explain: "TaskFailure would upload output files only when tasks fail (non-zero exit code). This is the opposite of what's needed—you want to store successfully converted files, not output from failed conversions. Using TaskFailure would mean successful conversions aren't saved."
    - id: "answer3"
...
You work as a Batch Processing Developer for MDFT Pro, a well-known training agency that provides certification courses to students globally. Claire, the Systems Integration Lead, has tasked you with developing an Azure Batch solution that processes and converts student assignment submissions stored in Azure Storage. Students upload various file formats including videos, documents, and presentations, which need to be converted to standardized formats for the grading system. The batch job processes files in batches of 100, running a conversion command for each file. During testing, you discovered that when one document has corrupted data and fails conversion (returning a non-zero exit code), you still need to ensure all other successfully converted files are uploaded to the output container. The requirement specifies that only successfully converted files should be placed in the container referenced by the outputContainerSasUrl parameter—failed conversions should not be stored to avoid filling the container with invalid data.
