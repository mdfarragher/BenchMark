...
answers:
    - id: "answer1"
      title: "COALESCE"
      correct: true
      explain: "The COALESCE function returns the first non-null value from a list of expressions. It evaluates AgentPrice first, then WholesalePrice, then ListPrice, returning the first one that is not NULL. This perfectly implements the TradePrice logic required."
    - id: "answer2"
      title: "CHOOSE"
...
SELECT 
    ProductID, 
    ________________ (AgentPrice, WholesalePrice, ListPrice) AS TradePrice
FROM Sales.Products
```
